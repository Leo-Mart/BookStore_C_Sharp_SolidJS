      ...
      <For each={props.toasts}>
        {(toast) => (
          <div class='flex pointer-events-auto items-center p-4 gap-3 min-w-60 max-w-80 rounded-lg bg-everforest-aqua text-everforest-bg-dim shadow-xl text-sm animate-toast-in'>
            ...
            <button class='bg-transparent border-none text-everforest-bg-dim text-lg leading-1 cursor-pointer opacity-70 hover:opacity-100' aria-label="dismiss notification" onClick={() => props.onDismiss(toast.id)}>
              &times;
            </button>
          </div>
        )}
