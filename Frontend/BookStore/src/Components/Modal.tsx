...
const Modal: Component<ModalProps> = (props: ModalProps) => {
  ...
  return (
    <Portal>
      ...
      <div role="dialog" aria-modal="true" aria-label={props.title} class={`fixed top-0 right-1/2 translate-x-1/2 z-50 flex w-full max-w-md flex-col bg-everforest-bg-dim shadow-2xl transition-transform druation-300 ease-in-out ${props.open ? 'translate-y-1/2' : 'top-full'}`}> 
        ...
        <div class="flex-1 overflow-y-auto px-2 py-2">{props.children}</div>
